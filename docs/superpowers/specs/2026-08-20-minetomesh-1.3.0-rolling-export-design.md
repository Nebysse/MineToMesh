...
游标按 `ChunkX → ChunkZ` 产生水平区块；单个区块内部按 `SectionY` 和方块坐标稳定排序。内存中只保留当前窗口及少量有界队列。

总区块数与总批次数通过边界差直接计算为 `long`。计算必须使用溢出检查；若 Minecraft 合法坐标仍导致计数无法表达，则请求失败并返回明确诊断，不能回绕成负数。
