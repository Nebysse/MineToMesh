...
public final class ResourceTextureExtractor {
    ...
    public static TextureProvider dynamicProvider() {
        return dynamicProvider(request -> {
            TextureManager manager = request.textureManager();
            return manager == null ? null : manager.getTexture(request.textureId());
        });
    }

    static TextureProvider dynamicProvider(
            java.util.function.Function<TextureProvider.Request, AbstractTexture> textureResolver) {
        ResourceTextureExtractor extractor = new ResourceTextureExtractor();
        return new TextureProvider() {
            @Override
            public String id() {
